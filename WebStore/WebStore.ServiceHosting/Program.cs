        ...
        {            
            ...
            using (var scope = host.Services.CreateScope())
            {
                ...
                try
                {
                    ...
                    if (!roleManager.RoleExistsAsync(WebStoreContants.Roles.User).Result)
                    {
                        var role = new IdentityRole(WebStoreContants.Roles.User);
                        var result = roleManager.CreateAsync(role).Result;
                    }
                    if (!roleManager.RoleExistsAsync(WebStoreContants.Roles.Admin).Result)
                    {
                        var role = new IdentityRole(WebStoreContants.Roles.Admin);
                        var result = roleManager.CreateAsync(role).Result;
                    }
                    ...
                    if (userStore.FindByEmailAsync("admin@mail.com", CancellationToken.None).Result == null)
                    {
                        ...
                        {
                            var roleResult = userManager.AddToRoleAsync(user, WebStoreContants.Roles.Admin).Result;
                        }
                    }
                }
                catch (Exception ex)
